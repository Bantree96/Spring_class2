...
@Component
public class ThumbFileUtils {
	public List<ThumbFileDto> parseFileInfo(int datasetIdx, MultipartFile multipartFile2){
		if(ObjectUtils.isEmpty(multipartFile2)) {
			return null;
		}
		
		System.out.println("*2");
		List<ThumbFileDto> fileList = new ArrayList<ThumbFileDto>();
		
		// 파일이 업로드될 폴더 생성
		DateTimeFormatter format = DateTimeFormatter.ofPattern("yyyyMMdd");
		ZonedDateTime current = ZonedDateTime.now(); 									// 현재날짜를 current로 가져옴
		String path = "src/main/resources/static/thumb/" + current.format(format); 	// images/현재날짜 폴더경로 정해줌
		String savePath = "/thumb/"+ current.format(format); 							// 이미지를 불러올 DB에 저장될 경로
		...
		// DB에 저장할 정보들
		ThumbFileDto dataset = new ThumbFileDto();
		...
		dataset.setStoredFilePath(savePath+"/" + multipartFile2.getOriginalFilename());
